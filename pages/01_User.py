...
def delete_confirm_one(x):
    ...
    util.flash("Erfolgreich gelöscht!")
...
if st.session_state.logged_in:
    ...
    if cols[1].button('Neuen User hinzufügen'):
        ...
        util.logger.info(f"User {st.session_state.username} hat einen neuen User hinzugefügt.")
        st.rerun()
            ...
            with st.expander(f"{x['name']}, {x['vorname']}", expanded = (True if x["_id"] == st.session_state.expanded else False)):
                with st.form(f'ID-{x["_id"]}'):
                    ...
                    selected_groups = st.pills(
                        ...
                        default=[gid for gid in x["groups"] if gid in group_name_by_id],
                        format_func=lambda gid: group_name_by_id[gid],
                        key=f'pills-{x["_id"]}',
                    )
                    ...
                    x_updated = {"name": name, "vorname": vorname, "rz": rz, "color" : color, "email": email, "groups": selected_groups, "kommentar": x['kommentar'], "bearbeitet": bearbeitet}
